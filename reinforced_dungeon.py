...
# ... (Image and sound loading remain the same)
...
def main():  # 메인 처리
    ...
    env = GameEnvironment()
    agent = QLearningAgent(state_size=1, action_size=env.action_size)
...
class GameEnvironment:
    ...
    def reset(self):
        global floor, pl_lifemax, pl_life, pl_str, food, potion, blazegem, pl_x, pl_y
        make_dungeon()
        put_event()
        floor = 1
        pl_lifemax = 300
    ...
    def get_state(self):
         # Simplified state representation (expand as needed)
        return (pl_x, pl_y, pl_life, food, potion, blazegem, floor, emy_life if "emy_life" in globals() else 0)


    def step(self, action):
        global idx, tmr, pl_x, pl_y, pl_life, food, potion, blazegem, treasure, emy_life, floor, fl_max

        reward = 0
        done = False

        if action < 4:  # Movement actions
            simulated_key = pygame.key.get_pressed() # Get the current key state
            simulated_key = list(simulated_key) # Convert to list for modification
            try:
                simulated_key[ACTIONS[action]] = 1 # Set the corresponding action key to pressed
            except IndexError:
                print(f"Invalid action: {action}, ACTIONS[action]: {ACTIONS[action]}") # Print debug information
                return self.get_state(), reward, done # Return without taking action
            move_player(simulated_key)
            if idx == 9:  # Game Over (from move_player)
                reward = -100
                done = True
            elif idx == 2:  # Next floor (from move_player)
                reward = 100
                # You might want to reset the environment here if the episode ends when reaching a new floor
                # For example: self.reset() and return self.get_state(), reward, done
            elif idx == 3:  # Found an item (from move_player)
                reward = 10            

        elif idx == 11:  # Battle actions
            handle_battle_action(action - 4) # Pass the correct battle command index (0-3)
            reward = 0 # Initialize reward
            if idx == 9:  # Game Over (from handle_battle_action)
                reward = -100
                done = True
            elif idx == 16:  # Win (from handle_battle_action)
                reward = 50
                done = True # Episode ends after battle
            elif idx == 15:  # Lose (from handle_battle_action)
                reward = -50
                done = True # Episode ends after battle
            elif idx == 13: # Enemy turn (from handle_battle_action)
                reward = -1 # Small negative reward for getting hit by the enemy
            elif idx == 11: # Still player's turn (used item)
                reward = -1 # Small negative reward for using an item

            return self.get_state(), reward, done
        return self.get_state(), reward, done  # Default return
...
class QLearningAgent:
    def __init__(self, state_size, action_size):
        self.state_size = state_size  # Store state_size
        self.action_size = action_size  # Store action_size  <--- This was missing
    ...
    def choose_action(self, state):
        ...
        if state not in self.q_table:
            self.q_table[state] = np.zeros(self.action_size) # Initialize Q-values for new states
...
# Function to handle battle actions and update game state
def handle_battle_action(btl_cmd):
    global idx, tmr, pl_life, emy_life, potion, blazegem
    # ... (Implement logic for attack, potion, blaze gem, run based on btl_cmd)
    # ... (Update pl_life, emy_life, potion, blazegem, idx, tmr accordingly)
    # ... (This function replaces the original battle logic in the main loop)


# Main game loop (modified)
def main():
    pygame.init()  # Make sure Pygame is fully initialized
    pygame.display.set_caption("One hour Dungeon")
    screen = pygame.display.set_mode((880, 720))  # Initialize the display  <-- MOVED UP
    clock = pygame.time.Clock()
    font = pygame.font.Font(None, 40)
    fontS = pygame.font.Font(None, 30)

    env = GameEnvironment()
    agent = QLearningAgent(len(env.get_state()), env.action_size)
    ...
    for episode in range(1000): # Number of episodes for training
        ...
        while not done:
            ...
            agent.learn(tuple(state), action, reward, tuple(next_state), done) # Convert state to tuple for indexing
            state = next_state

            # Render the game (update display, handle events, etc.)
            for event in pygame.event.get():
                if event.type == QUIT:
                    pygame.quit()
                    sys.exit()

            # ... (Rendering logic from the original game loop)

            pygame.display.update()
            clock.tick(4 + 2 * speed)  # Adjust speed as needed
