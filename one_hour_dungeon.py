...
TRE_NAME = ["동료애", "고객만족", "낮은 급여 spoiled.", "상여 +20", "상여 +100"]
EMY_NAME = [
...
def main():  # 메인 처리
    ...
    pygame.init()
    pygame.display.set_caption("신한은행 생존기")
    screen = pygame.display.set_mode((880, 720))
    clock = pygame.time.Clock()

    font = pygame.font.SysFont(["malgun", "gulim", "batang", "Arial Unicode MS"], 30)
    fontS = pygame.font.SysFont(["malgun", "gulim", "batang", "Arial Unicode MS"], 20)
    ...
    while True:
        ...
        if idx == 0:  # 타이틀 화면
            ...
            if fl_max >= 2:
                draw_text(screen, "다음단계에 도전? {}.".format(fl_max), 300, 460, font, CYAN)
            draw_text(screen, "생존위해 도전하시오", 320, 560, font, BLINK[tmr % 6])
            if key[K_SPACE] == 1:
                make_dungeon()
                put_event()
                floor = 1
            ...
            draw_text(screen, "몇 층? {} ({},{})".format(floor, pl_x, pl_y), 60, 40, fontS, WHITE)
            if welcome > 0:
                welcome = welcome - 1
                draw_text(screen, "환영합니다. {}.".format(floor), 300, 180, font, CYAN)
